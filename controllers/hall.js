...
let roomsDetails = [
  {
    ...
    room_booked_dates: [],
    amenities: "Single Bed, TV, WiFi, Non-AC, Storage",
    seats: 4,
    rent_per_hr: 2000,
  },
  {
    room_id: 2,
    room_no: 102,
    room_name: "Junior Suite Room",
    room_booked_dates: [],
    amenities: "Double-Bed, TV, WiFi, Storage, Non-AC, Iron Box, Hair Dryer",
    seats: 4,
    rent_per_hr: 5000,
  },
  {
    ...
    room_booked_dates: [],
    amenities:
      "King Size Bed, TV, WiFi, AC, Storage, Iron Box, Hair Dryer, Hot Tub",
    seats: 10,
    rent_per_hr: 10000,
  },
];
...
export const serverConnect = (req, res) => {
  res.status(200).json({
    message: `Hall Booking API Running, kindly check the API in Postman`,
  });
};
  ...
  try {
    ...
    if (index !== -1) {
      return res.status(404).json({
        message: `The Room No, that you have mentioned is already available. Please provide a different number.`,
      });
    }
...
export const getAllRoomDetails = (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `Data Fetched Suceesfully`, allRooms: roomsDetails });
  } catch (err) {
    //throw error if anything goes wrong
    res.status(500).json({
      message: `Internal Server Error!`,
...
export const roomBooking = (req, res) => {
  try {
    ...
    const roomIndex = roomsDetails.findIndex(
      (room) => room.room_id === room_id
    );
    if (roomIndex === -1) {
      return res
        .status(404)
        .json({ message: `There is no Room with the given id ${room_id}` });
    }

    //checking the room date availability
    let room = roomsDetails[roomIndex];
    let isBooked = room.room_booked_dates.some(
      (bookedDate) => bookedDate === date
    );
    if (isBooked) {
      return res.status(400).json({
        message: `Room ${room.room_no} is already booked in the date ${date}`,
      });
    }
    ...
    if (date < today) {
      return res
        .status(400)
        .json({ message: "Booking date must be today or in the future" });
    }

    //checking the booking room details, if the given room is booked
    let roomDateCheck = bookingRoom.some(
      (e) => e.date === date && e.room_id === room_id
    );
    if (roomDateCheck) {
      return res.status(404).json({
        message: `For the given room id : ${room_id}, the given date is not available`,
      });
    }
    ...
    res.status(200).json({
      message: `Room ${room.room_no} booked successfully`,
      roomBookedDetails: { room_name: room.room_name, ...bookedRoom },
    });
  } catch (err) {
    //throw error if anything goes wrong
    res.status(500).json({
...
export const getAllBookedRoomDetails = (req, res) => {
  try {
    ...
    if (bookingRoom.length === 0) {
      return res.status(404).json({ message: `No rooms are booked yet` });
    }
...
export const getAllCustomerWithRoomDetails = (req, res) => {
  try {
    ...
    res.status(200).json({
      message: `Data Fetched Suceesfully`,
      BookedRoomsCustomerDetails: customerDetails,
    });
  } catch (err) {
    //throw error if anything goes wrong
    res.status(500).json({
...
export const customerBookDetails = (req, res) => {
  try {
    ...
    let oneCustomerDetails = bookingRoom.filter(
      (bookedRoom) => bookedRoom.customer_name === customer_name
    );

    //throw error if the given customer name not found in the bookingRoom details
    if (oneCustomerDetails.length === 0) {
      return res.status(404).json({
        message: `Oops! ${customer_name} haven't booked any rooms yet`,
      });
    }
    ...
    res
      .status(200)
      .json({ message: `Data Fetched Suceesfully`, data: resCustomer });
  } catch (err) {
    //throw error if anything goes wrong
    res.status(500).json({
      message: `Internal Server Error!`,
...
export const pageNotFound = (req, res) => {
  res.status(404).json({
    message: `Page not found, please double check the API endpoint`,
  });
};
